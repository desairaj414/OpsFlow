...
| Entity/PII detection in text, transcripts, extractions | Local SLM via Ollama (+ regex first pass) | see `ollama list` output, [env-network.md](../reference/env-network.md) | Highest-sensitivity content never leaves the machine |
...
finding to log in [env-network.md](../reference/env-network.md) and escalate to the human — do not silently
