...
"initial TCS-hackathon-environment verification," not "Phase 0 acceptance criteria" now that phase-
based tracking is retired (see `state-progress.md`'s top note). Content below is otherwise the
original record, verified 2026-08-07, H+0:00–H+2:50 (Fri 09:00–11:50); dead links to the deleted
`prd-phase-0.md` removed, everything else kept as-written for the historical record. See
[env-network.md](env-network.md) for the full results table and
[models-routing.md](../architecture/models-routing.md) / [decisions-log.md](../decisions-log.md) for the model-
substitution record.
...
| Phi-4-reasoning (handbook smoke-test requirement, unused in primary path) | ⚠ INTERMITTENT | n/a | passed 3x, then failed, rechecked 3x back-to-back: 404, 404, 200 — gateway-side flakiness, not a deprecation. Non-blocking |
...
Full rationale and rejected alternatives: [decisions-log.md](../decisions-log.md).
...
(a local download cache — gitignored as of 2026-08-17 after an earlier session accidentally
committed its contents; see `decisions-log.md`).
...
- **Known deviation (resolved same session, see decisions-log.md):** frontend was Vite/React at
  this point; migrated to Next.js later the same hackathon, per the frozen PRD.
...
per the PRD's explicit "stop — do not wire anything yet" instruction regardless. (Never picked up
afterward — real-Jira wiring stayed out of scope; see `future-plans.md`.)
