...
# RB-001: Remediation — sharepoint-site
...
### Step 4
Wait 5 minutes and re-check both the alert status and the underlying health probe. **Only if X**: if the alert cleared but the health p

<!-- PAGE BREAK (simulated PDF page boundary — this step must stay atomic, see domain-guardrails.md) -->

robe has not recovered, do NOT close — mark `symptom_suppressed, root cause unconfirmed` and keep the incident open (Fake Fix Detector).
